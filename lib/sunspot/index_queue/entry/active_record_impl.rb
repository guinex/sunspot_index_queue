    ...
    module Entry
      ...
      class ActiveRecordImpl < ActiveRecord::Base
        ...
        class << self
          ...
          def add(klass, id, delete, priority, skip_check = false )
            queue_entry_key = {:record_id => id, :record_class_name => klass.name, :lock => nil}
            queue_entry = if delete || skip_check
                            where(queue_entry_key).first || new(queue_entry_key.merge(:priority => priority))
                          else
                            new(queue_entry_key.merge(:priority => priority))
                          end
          ...
          # Avoid the duplication of entries in queue and save number of requests to solr
          def new_record_ids(klass, ids)
            conditions = ["#{connection.quote_column_name('record_class_name')} = ? AND #{connection.quote_column_name('record_id')} IN (?)", klass.name, ids]
            queue_entry_record_ids = where(conditions).pluck('record_id')
            ids.find_all{|id| queue_entry_record_ids.include?(id)}.uniq
          end
